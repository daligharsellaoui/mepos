...
router.get('/', async (req, res) => {
    try {
        if (database_1.isDemoMode) {
            const data = database_1.demoDb.ingredient_losses.map(il => {
                const ing = database_1.demoDb.ingredients.find(i => i.id === il.ingredient_id);
                const dept = database_1.demoDb.departments.find(d => d.id === il.department_id);
                const reporter = database_1.demoDb.users.find(u => u.id === il.reported_by);
                return {
                    ...il,
...
router.post('/', async (req, res) => {
    ...
    if (database_1.isDemoMode) {
        try {
            const ingredient = database_1.demoDb.ingredients.find(i => i.id === parseInt(ingredient_id, 10));
            if (!ingredient) {
            ...
            const qtyDecimal = new decimal_js_1.default(quantity);
            const purchasePriceDecimal = new decimal_js_1.default(ingredient.purchase_price_per_unit);
            ...
            if (recipesUsingIng.length > 0) {
                ...
                const quantityNeeded = new decimal_js_1.default(mainRecipe.quantity_needed);
            ...
            let stockRow = database_1.demoDb.inventory_stocks.find(st => st.department_id === stockDeptId && st.ingredient_id === ingredient.id);
            if (!stockRow) {
                stockRow = {
            ...
            stockRow.quantity = new decimal_js_1.default(stockRow.quantity).minus(qtyDecimal).toNumber();
            // Log movement
            ...
            // Insert loss record
            const lossRecord = {
                id: database_1.demoDb.ingredient_losses.length + 1,
            ...
            database_1.demoDb.ingredient_losses.push(lossRecord);
    ...
    try {
       ...
       VALUES ($1, $2, 0.0000)
       ON CONFLICT (department_id, ingredient_id) DO NOTHING`, [stockDeptId, ingredient_id]);
        const lockStock = await client.query('SELECT quantity FROM inventory_stocks WHERE department_id = $1 AND ingredient_id = $2 FOR UPDATE', [stockDeptId, ingredient_id]);
        const currentQty = new decimal_js_1.default(lockStock.rows[0].quantity);
        const newQty = currentQty.minus(qtyDecimal);
        await client.query('UPDATE inventory_stocks SET quantity = $1, updated_at = CURRENT_TIMESTAMP WHERE department_id = $2 AND ingredient_id = $3', [newQty.toString(), stockDeptId, ingredient_id]);
        await client.query(`INSERT INTO stock_movements (department_id, ingredient_id, quantity, type, reference_id)
       VALUES ($1, $2, $3, 'loss', $4)`, [stockDeptId, ingredient_id, qtyDecimal.times(-1).toString(), 'loss_report']);
        const insertLoss = await client.query(`INSERT INTO ingredient_losses (department_id, ingredient_id, quantity, loss_reason, cost_loss, opportunity_loss, reported_by)
