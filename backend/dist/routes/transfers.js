...
router.post('/', async (req, res) => {
    ...
    const transferQty = new decimal_js_1.default(quantity);
    ...
    if (database_1.isDemoMode) {
        try {
            const srcDept = database_1.demoDb.departments.find(d => d.id === parseInt(source_department_id, 10));
            const destDept = database_1.demoDb.departments.find(d => d.id === parseInt(destination_department_id, 10));
            if (!srcDept || !destDept) {
            ...
            // Check source stock
            let srcStock = database_1.demoDb.inventory_stocks.find(st => st.department_id === srcDept.id && st.ingredient_id === parseInt(ingredient_id, 10));
            if (!srcStock) {
                srcStock = {
                ...
                database_1.demoDb.inventory_stocks.push(srcStock);
            }
            const srcQty = new decimal_js_1.default(srcStock.quantity);
            ...
            let destStock = database_1.demoDb.inventory_stocks.find(st => st.department_id === destDept.id && st.ingredient_id === parseInt(ingredient_id, 10));
            if (!destStock) {
                destStock = {
                    id: database_1.demoDb.inventory_stocks.length + 1,
                ...
                database_1.demoDb.inventory_stocks.push(destStock);
            }
            const destQty = new decimal_js_1.default(destStock.quantity);
            // Perform transfer
    ...
    try {
       ...
       VALUES ($1, $2, 0.0000)
       ON CONFLICT (department_id, ingredient_id) DO NOTHING`, [source_department_id, ingredient_id]);
        const srcStockRes = await client.query('SELECT quantity FROM inventory_stocks WHERE department_id = $1 AND ingredient_id = $2 FOR UPDATE', [source_department_id, ingredient_id]);
        const srcQty = new decimal_js_1.default(srcStockRes.rows[0].quantity);
        if (srcQty.lessThan(transferQty)) {
            throw new Error(`Stock insuffisant dans le dépôt de départ. Quantité disponible : ${srcQty.toString()}`);
        }
        await client.query(`INSERT INTO inventory_stocks (department_id, ingredient_id, quantity)
       VALUES ($1, $2, 0.0000)
       ON CONFLICT (department_id, ingredient_id) DO NOTHING`, [destination_department_id, ingredient_id]);
        const destStockRes = await client.query('SELECT quantity FROM inventory_stocks WHERE department_id = $1 AND ingredient_id = $2 FOR UPDATE', [destination_department_id, ingredient_id]);
        const destQty = new decimal_js_1.default(destStockRes.rows[0].quantity);
        const newSrcQty = srcQty.minus(transferQty);
        const newDestQty = destQty.plus(transferQty);
        await client.query('UPDATE inventory_stocks SET quantity = $1, updated_at = CURRENT_TIMESTAMP WHERE department_id = $2 AND ingredient_id = $3', [newSrcQty.toString(), source_department_id, ingredient_id]);
        await client.query('UPDATE inventory_stocks SET quantity = $1, updated_at = CURRENT_TIMESTAMP WHERE department_id = $2 AND ingredient_id = $3', [newDestQty.toString(), destination_department_id, ingredient_id]);
        const ref = `transfer-${source_department_id}-to-${destination_department_id}`;
        await client.query(`INSERT INTO stock_movements (department_id, ingredient_id, quantity, type, reference_id)
       VALUES ($1, $2, $3, 'transfer_out', $4)`, [source_department_id, ingredient_id, transferQty.times(-1).toString(), ref]);
        await client.query(`INSERT INTO stock_movements (department_id, ingredient_id, quantity, type, reference_id)
       VALUES ($1, $2, $3, 'transfer_in', $4)`, [destination_department_id, ingredient_id, transferQty.toString(), ref]);
...
router.get('/requests', async (req, res) => {
    try {
        if (database_1.isDemoMode) {
            const data = database_1.demoDb.transfer_requests.map(tr => {
                const ing = database_1.demoDb.ingredients.find(i => i.id === tr.ingredient_id);
                const srcDept = database_1.demoDb.departments.find(d => d.id === tr.source_department_id);
                const destDept = database_1.demoDb.departments.find(d => d.id === tr.destination_department_id);
                const requester = database_1.demoDb.users.find(u => u.id === tr.requested_by);
                const validator = database_1.demoDb.users.find(u => u.id === tr.validated_by);
                return {
                    ...tr,
...
router.post('/requests', async (req, res) => {
    ...
    const reqQty = new decimal_js_1.default(quantity);
    ...
    try {
        if (database_1.isDemoMode) {
            const srcDept = database_1.demoDb.departments.find(d => d.id === parseInt(source_department_id, 10));
            const destDept = database_1.demoDb.departments.find(d => d.id === parseInt(destination_department_id, 10));
            const ing = database_1.demoDb.ingredients.find(i => i.id === parseInt(ingredient_id, 10));
            if (!srcDept || !destDept || !ing) {
                return res.status(400).json({ status: 'error', message: 'Invalid department or ingredient ID' });
            }
            const newRequest = {
                id: database_1.demoDb.transfer_requests.length + 1,
            ...
            database_1.demoDb.transfer_requests.push(newRequest);
            return res.json({ status: 'success', data: newRequest });
        }
        const result = await (0, database_1.query)(`INSERT INTO transfer_requests (source_department_id, destination_department_id, ingredient_id, quantity, status, requested_by)
       VALUES ($1, $2, $3, $4, 'pending', $5) RETURNING *`, [source_department_id, destination_department_id, ingredient_id, reqQty.toString(), requested_by || null]);
 ...
 */
router.post('/requests/:id/validate', async (req, res) => {
    const { id } = req.params;
    const { validated_by } = req.body;
    if (database_1.isDemoMode) {
        try {
            const request = database_1.demoDb.transfer_requests.find(tr => tr.id === parseInt(id, 10));
            if (!request) {
            ...
            const transferQty = new decimal_js_1.default(request.quantity);
            // Check source stock
            let srcStock = database_1.demoDb.inventory_stocks.find(st => st.department_id === request.source_department_id && st.ingredient_id === request.ingredient_id);
            if (!srcStock) {
                srcStock = {
                    id: database_1.demoDb.inventory_stocks.length + 1,
            ...
            // Check destination stock
            let destStock = database_1.demoDb.inventory_stocks.find(st => st.department_id === request.destination_department_id && st.ingredient_id === request.ingredient_id);
            if (!destStock) {
                destStock = {
                    id: database_1.demoDb.inventory_stocks.length + 1,
    ...
    try {
        ...
        await client.query(`INSERT INTO inventory_stocks (department_id, ingredient_id, quantity)
       VALUES ($1, $2, 0.0000)
       ON CONFLICT (department_id, ingredient_id) DO NOTHING`, [request.source_department_id, request.ingredient_id]);
        const srcStockRes = await client.query('SELECT quantity FROM inventory_stocks WHERE department_id = $1 AND ingredient_id = $2 FOR UPDATE', [request.source_department_id, request.ingredient_id]);
        const srcQty = new decimal_js_1.default(srcStockRes.rows[0].quantity);
        if (srcQty.lessThan(transferQty)) {
            throw new Error(`Stock insuffisant dans le dépôt de départ. Quantité disponible : ${srcQty.toString()}`);
        }
        ...
        await client.query(`INSERT INTO inventory_stocks (department_id, ingredient_id, quantity)
       VALUES ($1, $2, 0.0000)
       ON CONFLICT (department_id, ingredient_id) DO NOTHING`, [request.destination_department_id, request.ingredient_id]);
        const destStockRes = await client.query('SELECT quantity FROM inventory_stocks WHERE department_id = $1 AND ingredient_id = $2 FOR UPDATE', [request.destination_department_id, request.ingredient_id]);
        const destQty = new decimal_js_1.default(destStockRes.rows[0].quantity);
        // Update stocks
        const newSrcQty = srcQty.minus(transferQty);
        const newDestQty = destQty.plus(transferQty);
        await client.query('UPDATE inventory_stocks SET quantity = $1, updated_at = CURRENT_TIMESTAMP WHERE department_id = $2 AND ingredient_id = $3', [newSrcQty.toString(), request.source_department_id, request.ingredient_id]);
        await client.query('UPDATE inventory_stocks SET quantity = $1, updated_at = CURRENT_TIMESTAMP WHERE department_id = $2 AND ingredient_id = $3', [newDestQty.toString(), request.destination_department_id, request.ingredient_id]);
        // Log movements
        const ref = `transfer-request-${id}`;
        await client.query(`INSERT INTO stock_movements (department_id, ingredient_id, quantity, type, reference_id)
       VALUES ($1, $2, $3, 'transfer_out', $4)`, [request.source_department_id, request.ingredient_id, transferQty.times(-1).toString(), ref]);
        await client.query(`INSERT INTO stock_movements (department_id, ingredient_id, quantity, type, reference_id)
       VALUES ($1, $2, $3, 'transfer_in', $4)`, [request.destination_department_id, request.ingredient_id, transferQty.toString(), ref]);
        // Update request status
        const updateReqRes = await client.query(`UPDATE transfer_requests 
       SET status = 'approved', validated_by = $1, updated_at = CURRENT_TIMESTAMP 
       WHERE id = $2 RETURNING *`, [validated_by || null, id]);
        await client.query('COMMIT');
...
router.post('/requests/:id/reject', async (req, res) => {
    ...
    try {
        ...
        const result = await (0, database_1.query)(`UPDATE transfer_requests 
       SET status = 'rejected', validated_by = $1, updated_at = CURRENT_TIMESTAMP 
       WHERE id = $2 AND status = 'pending' RETURNING *`, [validated_by || null, id]);
